...
function addLinkedCardCashIn(token, idempotencyKey, tenantId, accountType, ownerId, accountId, linkedCardId, cashInRequest) {
    return baseUrl
        .post(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/linkedCards/${linkedCardId}/cashIns`)
        .send(cashInRequest)
...
function findAllLinkedCardCashInFromCardId(token, idempotencyKey, tenantId, accountType, ownerId, accountId, linkedCardId, offset, limit) {
    return baseUrl
        .get(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/linkedCards/${linkedCardId}/cashIns?offset=${offset}&limit=${limit}`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${token}`)
        .expect('Content-Type', /json/)   
        .expect(200)
...
function findLinkedCardCashInFromTransactionId(token, idempotencyKey, tenantId, accountType, ownerId, accountId, linkedCardId, transactionId) {
    return baseUrl
        .get(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/linkedCards/${linkedCardId}/cashIns/${transactionId}`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${token}`)
        .expect('Content-Type', /json/)
        .expect(200)
...
function getLinkedCardCashInFee(token, idempotencyKey, tenantId, accountType, ownerId, accountId, linkedCardId, amount, currency) {
    return baseUrl
        .get(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/linkedCards/${linkedCardId}/cashInsFee?amount=${amount}&currency=${currency}`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${token}`)
        .expect('Content-Type', /json/)
        .then(result => {
