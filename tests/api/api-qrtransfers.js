...
function addQrCreditTransfer(token, idempotencyKey, tenantId, req) {
    return baseUrl
        .post(`/rest/account/tenants/${tenantId}/qrCreditTransfers`)
        .send(req)
...
function updateQrCreditTransfer(token, tenantId, qrCreditTransferId, req) {
     ...
     return baseUrl
        //.get(`/rest/account/tenants/${tenantId}/qrCreditTransfers/${qrCreditTransferId}`)
        .put(`/rest/account/tenants/${tenantId}/qrCreditTransfers/${qrCreditTransferId}`)
        .send(req)
        .set('Accept', 'application/json')
        //.set('Authorization', `Bearer ${token}`)
        //.expect('Content-Type', /json/)
