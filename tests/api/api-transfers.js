...
function addTransfer(token, idempotencyKey, tenantId, accountType, ownerId, accountId, req) {
    return baseUrl
        .post(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/transfers`)
        .send(req)
...
function findTransactionDetailedFromTransactionId(token, tenantId, accountType, ownerId, accountId, transactionId) {
    return baseUrl
        .get(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/transactions/${transactionId}/detailed`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${token}`)
        .expect('Content-Type', /json/)
        .expect(200)
...
function accountBalance(token, tenantId, accountType, ownerId, accountId) {
    return baseUrl
        .get(`/rest/account/tenants/${tenantId}/${accountType}/${ownerId}/accounts/${accountId}/balance`)
        .set('Accept', 'application/json')
        .set('Authorization', `Bearer ${token}`)
        .expect('Content-Type', /json/)
        .expect(200)
